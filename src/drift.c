...
// Defined down with the workspace-file writer it wraps, but called from the
// rename handler above it
static void RefreshCodeWorkspaceFile(const char* anchor);
...
void HandleRenameWorkspace() {
    ...
    // VS Code titles the window from the file's name, so a rename has to move
    // the generated workspace file too. Safe to load into the shared member
    // globals here for the same reason 'v' is: edit mode leaves this view to
    // arm itself, so nothing else is holding them
    char anchor[MAX_PATH];
    if (snprintf(anchor, MAX_PATH, "%s\\%s", workspaces_root, folder) < MAX_PATH) {
        LoadMembersFrom(anchor);
        if (json_block_reason == NULL) RefreshCodeWorkspaceFile(anchor);
    }
}
...
// Drift only ever rewrites or removes a .code-workspace it can prove it wrote.
// A hand-made one sharing the anchor is left alone, so "drift owns this file"
// stays a claim about this file rather than about the extension.
#define CODE_WORKSPACE_MARKER "// Generated by drift"

static bool IsGeneratedCodeWorkspace(const char* path) {
    FILE* f = fopen(path, "rb");
    ...
    if (!ok) return false;
    head[len] = '\0';
    return strstr(head, CODE_WORKSPACE_MARKER) != NULL;
}

// The generated workspace file in `anchor`, if there is one. Found by scanning
// rather than by recomputing the name, because the display name it was built
// from may have been renamed since.
static bool FindGeneratedCodeWorkspace(const char* anchor, char out[MAX_PATH]) {
    char search[MAX_PATH];
    if (snprintf(search, MAX_PATH, "%s\\*.code-workspace", anchor) >= MAX_PATH) {
        return false;
    }
    WIN32_FIND_DATA fd;
    HANDLE hFind = FindFirstFile(search, &fd);
    ...
    do {
        if (fd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY) continue;
        // A wildcard also matches 8.3 short names; confirm the real suffix
        size_t len = strlen(fd.cFileName);
        const char* suffix = ".code-workspace";
        size_t slen = strlen(suffix);
        if (len <= slen || _stricmp(fd.cFileName + len - slen, suffix) != 0) continue;
        if (strpbrk(fd.cFileName, "?*") != NULL) continue;
        char candidate[MAX_PATH];
        if (snprintf(candidate, MAX_PATH, "%s\\%s", anchor, fd.cFileName) >= MAX_PATH) {
            continue;
        }
        if (!IsGeneratedCodeWorkspace(candidate)) continue;
        strcpy(out, candidate);
        found = true;
...
// Keep an already-generated workspace file matching the member list. Called
// wherever membership or the display name changes, so the file tracks
// settings.json rather than tracking the last press of 'v'.
//
// Deliberately does nothing when no generated file exists: 'v' is what creates
// one, so a workspace never opened in VS Code never acquires the file. Requires
// members[] to hold this anchor's list.
//
// Best effort. The member change itself already succeeded and is the thing that
// matters, and 'v' rebuilds this file from scratch every time, so a failure
// here heals on the next open rather than persisting.
static void RefreshCodeWorkspaceFile(const char* anchor) {
    char existing[MAX_PATH];
    if (!FindGeneratedCodeWorkspace(anchor, existing)) return;

    char display[MAX_PATH];
    WorkspaceDisplayName(AnchorFolder(anchor), display, sizeof(display));

    char written[MAX_PATH];
    if (!WriteCodeWorkspaceFile(anchor, display, written)) return;
    // A rename moves the file, since the name is what VS Code titles the
    // window with. Drop the version under the old name rather than leave two
    if (_stricmp(existing, written) != 0) DeleteFile(existing);
}
...
enum MemberChangeResult ApplyMemberChange(
    ...
    if (SaveMembersTo(anchor)) {
        ...
        // Still holding the lock, so the generated workspace file is rebuilt
        // from exactly the list that just reached settings.json rather than
        // from whatever a concurrent Drift writes next
        RefreshCodeWorkspaceFile(anchor);
    } else {
